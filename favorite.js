        ...
        <div class="mb-2">
          <div class="card">
            ...
            <div class="card-footer">
              ...
              <button class="btn btn-danger btn-add-favorite"data-id ='${item.id
        }'>X</button>
            </div>
          </div>
        </div>
...
const feature = {
  ...
  removeFavorite(id){
    let movieItem = movies.findIndex((movie) =>movie.id === id)
    movies.splice(movieItem, 1)
...
page.pager(movies.length)
version.A(movies)
//more、removeFavorite監聽器
dataPanel.addEventListener("click", (event) => {
  if (event.target.matches('.btn-show-movie')) {
    feature.flashWindos(event.target.dataset.id);
  } else if (event.target.matches('.btn-add-favorite')) {
    feature.removeFavorite(Number(event.target.dataset.id))
  }
});
//版型切換監聽器
